...
const Mobilenav = dynamic(() => import("./mobilenav.jsx"), { ssr: false });
  ...
  return (
    <div className="flex justify-center">
      <nav className="backdrop-blur-md bg-opacity-50 bg-gray-800 rounded-lg shadow-lg fixed w-full z-10 px-4 py-3">
        ...
        {/* Bottom Info Bar */}
        {open && (
          <div className="mt-4 bg-[#B852D7] bg-opacity-35 p-1 hidden lg:flex justify-center items-center rounded-md">
            <div className="flex items-center gap-4">
              <TbBus className="text-yellow-400 text-xl" />
              <p className="text-white text-sm">
                Lorem ipsum dolor sit amet, consectetur adipisicing elit.
              </p>
              <button className="bg-white text-black text-xs px-2 py-1 rounded flex items-center gap-2">
                Try it now <MdArrowOutward className="text-black w-4 h-4" />
              </button>
            </div>
            <RxCross2
              onClick={() => setOpen(false)}
              className="text-white ml-28 text-lg font-bold cursor-pointer"
            />
          </div>
        )}
      </nav>
