...
const Chat = ({ isFirstMessage, onSendMessage }) => {
  ...
  return (
    <>
      <div className="h-auto w-full bg-zinc-900 flex flex-col justify-center items-center px-4 pt-32 sm:pt-0">
        {isFirstMessage ? (
        ...
        <div className="flex flex-col justify-center items-center fixed w-full px-4">
          {isFirstMessage ? (
            <div className="flex flex-col sm:flex-row sm:gap-2 lg:gap-4 mt-32">
              {["What is the best place in Varanasi?", "Where can I find good food?", "What is the history of Varanasi?", "How to explore Varanasi?"].map(
                (text, index) => (
                  <div
                    key={index}
